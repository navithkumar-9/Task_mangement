...
const Dashboard = () => {
  ...
  const members = JSON.parse(localStorage.getItem('admin_members_list') || '[]');
    ...
    <div className="page">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Welcome back, <span className="text-accent">{profile?.username || user?.username || 'User'}</span></p>
        </div>
      </div>
        ...
        <div className="content-card">
          ...
          {tasks.length === 0 ? (
            <div className="empty-state">
              <p>No tasks yet</p>
              <span className="empty-hint">Go to "Tasks" to create your first task</span>
            </div>
          ) : (
              ...
              <table className="data-table">
                ...
                <tbody>
                  {tasks.slice(0, 5).map((t, i) => (
                    <tr key={i}>
                      <td className="text-bold">{t.title}</td>
                      <td><span className={`priority-badge priority-${t.priority}`}>{t.priority}</span></td>
                      <td><span className={`status-badge status-${t.status}`}>{t.status.replace('-', ' ')}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="content-card">
            ...
            {members.length === 0 ? (
              <div className="empty-state">
                ...
                <span className="empty-hint">Go to "Create Member" to add one</span>
              </div>
            ) : (
              <div className="table-wrapper">
                <table className="data-table">
