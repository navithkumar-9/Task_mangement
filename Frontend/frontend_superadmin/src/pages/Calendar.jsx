...
const priorityColors = {
  HIGH: { bg: 'rgba(255,107,107,0.12)', color: '#ff6b6b', border: 'rgba(255,107,107,0.3)' },
  MEDIUM: { bg: 'rgba(255,185,70,0.12)', color: '#ffb946', border: 'rgba(255,185,70,0.3)' },
  LOW: { bg: 'rgba(75,207,130,0.12)', color: '#4bcf82', border: 'rgba(75,207,130,0.3)' },
};
...
const TaskDetailModal = ({ tasks, date, onClose }) => {
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(4px)' }}
      onClick={onClose}
    >
      <div onClick={(e) => e.stopPropagation()} style={{ background: '#ffffff', borderRadius: '16px', width: '520px', maxHeight: '80vh', overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.15)', animation: 'slideUp 0.25s ease-out' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h2 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 700 }}>Tasks Due</h2>
            <p style={{ margin: '4px 0 0', fontSize: '0.8rem', color: 'var(--text-muted)' }}>{dateStr}</p>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', fontSize: '1.4rem', cursor: 'pointer', color: 'var(--text-muted)', padding: '4px 8px', borderRadius: '6px' }}>&times;</button>
        </div>
...
const Calendar = () => {
  ...
  const calendarDays = useMemo(() => {
    ...
    for (let i = startPad - 1; i >= 0; i--) {
      days.push({ day: prevMonthLast - i, isCurrentMonth: false, date: new Date(currentYear, currentMonth - 1, prevMonthLast - i) });
    }
  ...
  const getDateKey = (d) => {
    const y = d.date.getFullYear(), m = String(d.date.getMonth() + 1).padStart(2, '0'), dd = String(d.date.getDate()).padStart(2, '0');
    return `${y}-${m}-${dd}`;
  };
  ...
  const handleDayClick = (dayObj) => {
    ...
    if (dayTasks.length > 0) { setSelectedDate(dayObj.date); setSelectedTasks(dayTasks); }
  };

  const isToday = (d) => d.date.getDate() === today.getDate() && d.date.getMonth() === today.getMonth() && d.date.getFullYear() === today.getFullYear();
    ...
    <div className="page">
      ...
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <p className="page-subtitle">Track all task deadlines across the organization</p>
        </div>
        {teamMembers.length > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <label style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-muted)' }}>Team Member:</label>
            <select
              value={memberFilter}
              onChange={(e) => setMemberFilter(e.target.value)}
              style={{ padding: '8px 14px', borderRadius: '8px', border: '1px solid var(--border-color)', fontSize: '0.88rem', fontWeight: 500, color: 'var(--text-primary)', background: '#fff', cursor: 'pointer', outline: 'none', minWidth: '180px' }}
            >
              <option value="">All Members</option>
              {teamMembers.map((name) => <option key={name} value={name}>@{name}</option>)}
            </select>
            {memberFilter && (
              <button onClick={() => setMemberFilter('')} style={{ background: 'transparent', border: 'none', color: 'var(--primary)', fontWeight: 600, cursor: 'pointer', fontSize: '0.85rem' }}>Clear</button>
            )}
          </div>
        )}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
        {[
          { label: 'Total Deadlines', value: stats.total, color: '#49CCF9', bg: 'rgba(73,204,249,0.08)', icon: '📋' },
          { label: 'Upcoming', value: stats.upcoming, color: '#7B68EE', bg: 'rgba(123,104,238,0.08)', icon: '🔜' },
          { label: 'Completed', value: stats.completed, color: '#4bcf82', bg: 'rgba(75,207,130,0.08)', icon: '✅' },
          { label: 'Overdue', value: stats.overdue, color: '#ff6b6b', bg: 'rgba(255,107,107,0.08)', icon: '⚠️' },
        ].map((stat) => (
          <div key={stat.label} style={{ background: '#ffffff', borderRadius: '12px', padding: '20px', border: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: '14px', boxShadow: 'var(--shadow-sm)' }}>
            <div style={{ width: '44px', height: '44px', borderRadius: '10px', background: stat.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.2rem', flexShrink: 0 }}>{stat.icon}</div>
            <div>
      ...
      <div style={{ background: '#ffffff', borderRadius: '16px', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-sm)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            <button onClick={() => navigateMonth(-1)} style={{ width: '36px', height: '36px', borderRadius: '8px', border: '1px solid var(--border-color)', background: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-secondary)' }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="15 18 9 12 15 6" /></svg>
            </button>
            <h2 style={{ margin: 0, fontSize: '1.15rem', fontWeight: 700, minWidth: '200px', textAlign: 'center' }}>{MONTHS[currentMonth]} {currentYear}</h2>
            <button onClick={() => navigateMonth(1)} style={{ width: '36px', height: '36px', borderRadius: '8px', border: '1px solid var(--border-color)', background: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-secondary)' }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="9 18 15 12 9 6" /></svg>
            </button>
          </div>
          <button onClick={goToToday} style={{ padding: '8px 16px', borderRadius: '8px', border: '1px solid var(--border-color)', background: 'var(--primary-light)', color: 'var(--primary)', fontWeight: 600, fontSize: '0.82rem', cursor: 'pointer' }}>Today</button>
        </div>

        {/* Legend */}
        <div style={{ padding: '12px 24px', borderBottom: '1px solid var(--border-color)', display: 'flex', gap: '20px', alignItems: 'center', background: 'var(--bg-body)' }}>
          <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 600 }}>PRIORITY:</span>
          {[{ label: 'High', color: '#ff6b6b' }, { label: 'Medium', color: '#ffb946' }, { label: 'Low', color: '#4bcf82' }].map((item) => (
        ...
        {loading ? (
          <div style={{ padding: '80px', textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--text-muted)', marginTop: '12px' }}>Loading calendar...</p>
          </div>
        ) : (
          <div style={{ padding: '16px 24px 24px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '4px', marginBottom: '8px' }}>
              {DAYS.map((day) => (
                <div key={day} style={{ textAlign: 'center', padding: '8px', fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{day}</div>
              ))}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '4px' }}>
              {calendarDays.map((dayObj, idx) => {
                ...
                const highestPriority = dayTasks.reduce((acc, t) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      border: todayClass ? '2px solid var(--primary)' : '1px solid var(--border-light)',
                      background: todayClass ? 'var(--primary-light)' : hasTasks ? '#fafbfc' : '#fff',
                      opacity: dayObj.isCurrentMonth ? 1 : 0.4,
                      cursor: hasTasks ? 'pointer' : 'default',
                      transition: 'all 0.15s ease', position: 'relative',
                      ...(hasTasks && { boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }),
                    }}
                    onMouseEnter={(e) => { if (hasTasks) { e.currentTarget.style.transform = 'translateY(-1px)'; e.currentTarget.style.boxShadow = '0 4px 12px rgba(0,0,0,0.08)'; } }}
                    onMouseLeave={(e) => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = hasTasks ? '0 1px 3px rgba(0,0,0,0.04)' : 'none'; }}
                  >
                    <div style={{ fontSize: '0.82rem', fontWeight: todayClass ? 700 : 500, color: todayClass ? 'var(--primary)' : dayObj.isCurrentMonth ? 'var(--text-primary)' : 'var(--text-muted)', marginBottom: '4px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <span>{dayObj.day}</span>
                      {hasTasks && (
                        <span style={{ fontSize: '0.65rem', fontWeight: 700, color: '#fff', background: hasOverdue ? '#ff6b6b' : (priorityColors[highestPriority]?.color || 'var(--primary)'), borderRadius: '10px', padding: '1px 6px', lineHeight: '1.3' }}>{dayTasks.length}</span>
                      )}
                    </div>
                    ...
                    {dayTasks.length > 3 && <div style={{ fontSize: '0.62rem', color: 'var(--primary)', fontWeight: 600, paddingTop: '2px' }}>+{dayTasks.length - 3} more</div>}
                  </div>
                );
              })}
            </div>
      ...
      {/* Upcoming Deadlines */}
      <div style={{ marginTop: '24px', background: '#ffffff', borderRadius: '16px', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-sm)', overflow: 'hidden' }}>
              ...
              return (
                <div key={task.id} style={{ display: 'flex', alignItems: 'center', justifycontent: 'space-between', padding: '12px 0', borderBottom: '1px solid var(--border-light)' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
                    <span style={{ padding: '3px 8px', borderRadius: '6px', fontSize: '0.7rem', fontWeight: 600, background: sStyle.bg, color: sStyle.color }}>{sStyle.label}</span>
                    <span style={{ fontSize: '0.78rem', fontWeight: 600, color: diffDays <= 1 ? '#ff6b6b' : diffDays <= 3 ? '#ffb946' : 'var(--text-muted)', minWidth: '80px', textAlign: 'right' }}>
      ...
      {selectedDate && selectedTasks.length > 0 && (
        <TaskDetailModal tasks={selectedTasks} date={selectedDate} onClose={() => { setSelectedDate(null); setSelectedTasks([]); }} />
      )}
